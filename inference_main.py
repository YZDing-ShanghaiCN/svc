...
def main():
    ...
    parser.add_argument('-ip', '--input_path', type=str, default=None, help='输入音频路径，指定后不再从raw文件夹读取')
    parser.add_argument('-op', '--output_path', type=str, default=None, help='输出音频路径，指定后不再保存到results文件夹')
    ...
    input_names = [input_path] if input_path else clean_names
    infer_tool.fill_a_to_b(trans, input_names)
    for clean_name, tran in zip(input_names, trans):
        if input_path:
            raw_audio_path = clean_name
            output_name = os.path.basename(clean_name)
        else:
        ...
        for spk in spk_list:
            ...
            if output_path:
                ...
                res_path = f'results/{output_name}_{key}_{spk}{cluster_name}_{isdiffusion}_{f0p}.{wav_format}'
